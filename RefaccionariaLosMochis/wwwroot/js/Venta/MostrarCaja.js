...
function manejarRespuestaObtenerVentas(response) {
    if (response.lista && response.lista.length > 0) {
        ...
        response.lista.forEach(function (item) {
            let idVenta = item.idVenta || '';
            let nombreCliente = item.nombreCliente || 'N/A';
            let nombreVendedor = item.nombreVendedor || 'N/A';
            let cantidadProductos = item.cantidadProductos || 0;
            let totalPagar = (typeof item.totalPagar === 'number' && !isNaN(item.totalPagar)) ? item.totalPagar.toFixed(2) : '0.00';
